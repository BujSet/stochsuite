...
int main(int argc, char* argv[]) {
    ...
    for (size_t i = 0; i < niters; i++) {
        ...
        asm volatile(".globl __stoch_br_tailwag_0\n__stoch_br_tailwag_0:");
        if(distNum < dist3P){
            distCycle = std::llround(rng->lognormal_distribution(std::log(dist3Ave),dist3V));
        } else {
            asm volatile(".globl __stoch_br_tailwag_1\n__stoch_br_tailwag_1:");
            if(distNum < (dist3P + dist2P) ){
        ...
        asm volatile(".globl __stoch_br_tailwag_2\n__stoch_br_tailwag_2:");
        if(noiseNum < noiseP){
