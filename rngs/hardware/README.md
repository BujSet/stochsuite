...
### Extracting Chip Area and Critical Path Delay Under Relistic Fanout Constraints

Look at modify files `synth_netlist.ys` and `run_sta.tcl` to select which design to examine 
and which technology node to use. Current flow has been tested on the vsclib013 130nm library, 
as well as 45nm and 15nm libraries from OpenCell and FreePDK, respectively. Area and critical 
path delay estimates can be viewed from the reports generated by the following commands:

```
yosys synth_netlist.ys
../../OpenSTA/build/sta run_sta.tcl
```
